...
class CodegenMYNN : public MemoizedExprTranslator<std::vector<Output>>, public CodegenCBase {
 public:
  explicit CodegenMYNN(const std::string& id) { this->ext_func_id_ = id; }

  std::vector<Output> VisitExprDefault_(const Object* op) final {
    LOG(FATAL) << "MYNN codegen doesn't support: " << op->GetTypeKey();
    return {};
  }
 ...
 private:
  ...
  GenerateBodyOutput GenerateOpCall(const CallNode* call) {
    ...
    static const std::map<std::string, std::pair<std::string, ArgFunType>> op_map = {
        {"nn.conv2d", {"mynn_conv2d", Conv2d}},
        {"nn.relu", {"mynn_relu", Relu}},
        {"add", {"mynn_add", Add}},
    };
  ...
  friend class MYNNModuleCodegen;
};
 ...
 */
class MYNNModuleCodegen : public CSourceModuleCodegenBase {
 public:
  // Create a corresponding DNNL function for the given relay Function.
  std::pair<std::string, Array<String>> GenMYNNFunc(const Function& func) {
    ICHECK(func.defined()) << "Input error: expect a Relay function.";

    // Record the external symbol for runtime lookup.
    auto sid = GetExtSymbol(func);

    CodegenMYNN builder(sid);
  ...
  runtime::Module CreateCSourceModule(const ObjectRef& ref) override {
    ...
    code_stream_ << "#include <mynn/mynn_kernel.h>\n";
    code_stream_ << "using namespace tvm::runtime;\n";
    code_stream_ << "using namespace tvm::runtime::contrib;\n";
    code_stream_ << "\n";

    ICHECK(ref->IsInstance<FunctionNode>());
    auto res = GenMYNNFunc(Downcast<Function>(ref));
 ...
 */
runtime::Module MYNNCompiler(const ObjectRef& ref) {
  MYNNModuleCodegen mynn;
  return mynn.CreateCSourceModule(ref);
}

TVM_REGISTER_GLOBAL("relay.ext.mynn").set_body_typed(MYNNCompiler);
